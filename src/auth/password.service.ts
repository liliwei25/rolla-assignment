...
const UNDEFINED_SALT_OR_ROUNDS_ERROR = `${BCRYPT_SALT_KEY} is not defined`;
...
export class PasswordService {
  ...
  constructor(private configService: ConfigService) {
    const saltOrRounds = this.configService.get<string>(BCRYPT_SALT_KEY);
  ...
  private static parseSalt(value: string | undefined): Salt {
    if (!value) {
